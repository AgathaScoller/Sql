...
UPDATE filmes SET categoria_id = (SELECT id FROM categorias WHERE nome = 'Drama') WHERE titulo = 'Joker' OR titulo = 'Parasite' OR titulo = 'The Godfather';
...
-- Tabela assentos (lugares) de cada sala
CREATE TABLE IF NOT EXISTS assentos (
    id INT AUTO_INCREMENT PRIMARY KEY,
    sala_id INT NOT NULL,
    linha CHAR(1) NOT NULL, -- Exemplo: A, B, C
    numero INT NOT NULL, -- Exemplo: 1, 2, 3
...
-- Repita para outras salas se quiser

-- Tabela reservas de assentos para sessões
...
-- Tabela usuários para funcionários (login simples)
...
-- Alterar tabela produtos para incluir fornecedor_id
ALTER TABLE produtos
ADD COLUMN IF NOT EXISTS fornecedor_id INT,
ADD CONSTRAINT IF NOT EXISTS fk_produtos_fornecedor FOREIGN KEY (fornecedor_id) REFERENCES fornecedores(id);
...
ALTER TABLE reservas ADD COLUMN status ENUM('ativa', 'cancelada', 'modificada') DEFAULT 'ativa';
DELIMITER //
